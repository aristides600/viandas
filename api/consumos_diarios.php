...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        foreach ($pacientesDietas as $consumo) {
            ...
            // Verificar si ya existe un consumo para la comida seleccionada
            $verificarSql = "SELECT dieta_id, postre_id, acompaniante 
                             FROM consumos_diarios 
                             WHERE internacion_id = :internacion_id 
                             AND fecha_consumo = CURRENT_DATE 
            ...
            if ($existeConsumo) {
                // Verificar si hay alguna diferencia entre los datos actuales y los nuevos
                $camposActualizar = [];

                if ($existeConsumo['dieta_id'] != $dieta_id) {
                    $camposActualizar[] = 'dieta_id = :dieta_id';
                }
                if ($existeConsumo['postre_id'] != $postre_id) {
                    $camposActualizar[] = 'postre_id = :postre_id';
                }
                if ($existeConsumo['acompaniante'] != $acompaniante) {
                    $camposActualizar[] = 'acompaniante = :acompaniante';
                }

                // Si hay diferencias, realizar el UPDATE
                if (!empty($camposActualizar)) {
                    $updateSql = "UPDATE consumos_diarios SET " . implode(', ', $camposActualizar) . "
                                  WHERE internacion_id = :internacion_id 
                                  AND fecha_consumo = CURRENT_DATE 
                                  AND comida_id = :comida_id";
                    $stmtUpdate = $conn->prepare($updateSql);
                    ...
                    if (in_array('dieta_id = :dieta_id', $camposActualizar)) {
                        $params[':dieta_id'] = $dieta_id;
                    }
                    if (in_array('postre_id = :postre_id', $camposActualizar)) {
                        $params[':postre_id'] = $postre_id;
                    }
                    if (in_array('acompaniante = :acompaniante', $camposActualizar)) {
                        $params[':acompaniante'] = $acompaniante;
                    }
                ...
                // Insertar nuevo registro si no existe
                $insertSql = "INSERT INTO consumos_diarios (
                                internacion_id, postre_id, dieta_id, fecha_consumo, comida_id, 
                                cantidad, acompaniante, usuario_id, estado, sector_id
                              ) 
                              VALUES (
                                :internacion_id, :postre_id, :dieta_id, NOW(), :comida_id, 
                                1, :acompaniante, :usuario_id, 1, :sector_id
                              )";
                $stmtInsert = $conn->prepare($insertSql);
                $stmtInsert->execute([
                    ':internacion_id' => $internacion_id,
