...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        // Consulta para obtener registros de pacientes_dietas donde colacion_id > 1 y estado = 1
        $sql = "SELECT pd.internacion_id, pd.colacion_id, pd.acompaniante, i.sector_id
                FROM pacientes_dietas pd
                JOIN internaciones i ON pd.internacion_id = i.id
                WHERE pd.estado = 1 AND pd.colacion_id > 1";
        ...
        foreach ($pacientesDietas as $consumo) {
            ...
            $colacion_id = $consumo['colacion_id'];
            $sector_id = $consumo['sector_id'];

            // Verificar si ya existe un consumo para el internacion_id y colacion_id en las últimas 3 horas
            $verificarSql = "SELECT id, cantidad 
                             FROM consumos_colaciones 
                             WHERE internacion_id = :internacion_id 
                             AND colacion_id = :colacion_id 
            ...
            if ($registroExistente) {
                ...
                $updateSql = "UPDATE consumos_colaciones 
                ...
                $insertSql = "INSERT INTO consumos_colaciones 
                              (internacion_id, sector_id, colacion_id, cantidad, fecha_consumo, usuario_id, estado)
                              VALUES 
                              (:internacion_id, :sector_id, :colacion_id, 1, NOW(), :usuario_id, 1)";
